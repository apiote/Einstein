...
public class Einstein extends Application {
            ...
            try {
                ...
                while (!allVotes) {
                    ...
                    if (response.split(" ")[0].equals("success")
                        ...
                        throw new IllegalStateException(response.split(" ")[3]); //todo or 5
                }
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public void receiveMoveVotes(BoardActivity context) {
        /*Thread t = new Thread(() -> { //todo protocol
            try {
                int a, b;
                boolean allVotes = false;
                while (!allVotes) {
                    ...
                    if (response.split(" ")[0].equals("success")
                            ...
                            && !(response.split(" ")[5].equals("selected")
                            || (response.split(" ")[5].equals("not")
                            && response.split(" ")[6].equals("selected")))) {
                        a = Integer.parseInt(response.split(" ")[3]);
                        b = Integer.parseInt(response.split(" ")[4]);
                        context.acknowledgeNewMoveVote(a, b, false);
                    } else if (response.split(" ")[5].equals("selected")
                            || (response.split(" ")[5].equals("not")
                            && response.split(" ")[6].equals("selected"))) {
                        a = Integer.parseInt(response.split(" ")[3]);
                        b = Integer.parseInt(response.split(" ")[4]);
